...
def save_data(df_instr, df_tc_usage, df_tc_inv):
    """Excel をアトミックに保存（PermissionError 対策で置換 + リトライ）。"""
...
st.set_page_config(page_title="計測器管理アプリ v4.4 r4c (フォーム編集 + オートセーブ)", layout="wide")

# ------------------------------------
# Sidebar（ログイン持続 / ページ選択保持）
# ------------------------------------
with st.sidebar:
    st.title("計測器管理")
...
# ---------------- 管理者ページ ----------------
    ...
    # === 計測器マスタの一括編集（管理者）: 編集バッファ＋2択UI＋保存時正規化 + オートセーブ ===
    st.divider()
    st.subheader("計測器マスタの一括編集（管理者）")

    # 編集バッファをセッションに保持（初回のみ作成）
    ...
    with st.expander("オートセーブ（ベータ）", expanded=False):
        st.checkbox("オートセーブを有効にする", key="autosave_enabled",
                    help="『編集を反映（未保存）』後、一定間隔でExcelへ自動保存します（スクロール位置は維持されます）。")
    ...
    with st.form("master_bulk_form"):
        ...
        col_em1, col_em2, col_em3, col_em4 = st.columns([1,1,1,1])
        apply_clicked = col_em1.form_submit_button("編集を反映（未保存）")
        save_clicked  = col_em2.form_submit_button("Excelへ保存", type="primary")
        discard_clicked = col_em3.form_submit_button("未保存編集を破棄して元に戻す")
        # 便利ボタン：オートセーブON時は apply と同時に保存も走らせる
        quick_clicked = col_em4.form_submit_button("反映して即保存（推奨）")

    # フォーム送信後の処理（どのボタンかで分岐）
    if apply_clicked or save_clicked or quick_clicked:
        tmp = st.session_state.get("editor_master")
        if tmp is None:
            tmp = edited_master
        ...
        if tmp['識別番号'].isna().any() or (tmp['識別番号'].astype(str).str.strip() == '').any():
            ...
            # まずバッファに反映（ロックで保護）
            with st.session_state.autosave_lock:
                st.session_state.master_edit_buf = tmp.copy()
            st.session_state.master_dirty = True
            st.success('未保存の編集内容をバッファに反映しました。')

            # 即保存を選んだ場合、またはオートセーブ有効かつ「反映のみ」の時に即時保存したい場合
