...
# 管理者
    ...
    # ====== 計測器マスタの一括編集（フォーム + オートセーブ + dict安全） ======
    st.divider()
    ...
    # ▼ 無名列（1・2列目＝記載なし）と 6列目＝貸出状況 を非表示
    def is_nameless(col: str) -> bool:
        c = str(col).strip()
        return (c == "") or c.lower().startswith("unnamed:")

    all_cols = list(df_master_edit.columns)
    nameless_cols = [c for c in all_cols if is_nameless(c)]
    nameless_to_hide = nameless_cols[:2]  # 左から2本まで

    hidden_cols = set(nameless_to_hide + (['貸出状況'] if '貸出状況' in all_cols else []))
    ...
    with st.form("master_bulk_form"):
        edited_master = st.data_editor(
            ...
            column_config={
                ...
                "校正係数": st.column_config.NumberColumn(min_value=0.0, step=0.01, help="機器ごとの構成係数（実数）。"),
            },
        )
        ...
        if apply_clicked or save_clicked or quick_clicked:
            ...
            # 構成係数 数値化（可視にある場合のみ）
            if '校正係数' in tmp_df.columns:
                tmp_df['校正係数'] = pd.to_numeric(tmp_df['校正係数'], errors='coerce')
            ...
            with st.session_state.autosave_lock:
                ...
                if key in base.columns and key in tmp_df.columns:
                    ...
                    tmp_df = tmp_df.set_index(key)
                    for col in tmp_df.columns:
                        base.loc[tmp_df.index, col] = tmp_df[col]
