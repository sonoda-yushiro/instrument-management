...
# 校正係数（旧: 構成係数）
NEW_FIELDS = ["メーカー","購入日","校正期限","校正係数1","校正係数1_単位",
    "校正係数2","校正係数2_単位"]
...
# 2) 旧「校正係数」「校正係数_単位」があれば、それを「校正係数1」側へ移行
        if "校正係数" in df_instr.columns and df_instr["校正係数"].notna().any():
    # 数値化してからコピー（文字列や空は NaN に）
           df_instr["校正係数1"] = pd.to_numeric(df_instr["校正係数"], errors="coerce")

        if "校正係数_単位" in df_instr.columns and df_instr["校正係数_単位"].astype(str).str.strip().ne("").any():
           df_instr["校正係数1_単位"] = (
           df_instr["校正係数_単位"].astype(str).replace("nan","").fillna("")
    )
...
# 係数は数値化、単位は文字列として整形
    ...
    if "校正係数" in df_instr.columns:
        df_instr["校正係数"] = pd.to_numeric(df_instr["校正係数"], errors="coerce")
    else:
        df_instr["校正係数"] = pd.Series(dtype='float')
    
    # 追加：校正係数_単位 列を文字列で保証
    if "校正係数_単位" not in df_instr.columns:
        df_instr["校正係数_単位"] = ""
    else:
        df_instr["校正係数_単位"] = (
        df_instr["校正係数_単位"].astype(str).replace("nan", "").fillna("")
...
# タイトル文言は据え置き、起動ページは「貸出状況（=Dashboardページ）」
st.set_page_config(page_title="計測器管理表", layout="wide")
...
with st.sidebar:
    ...
    if 'page' not in st.session_state:
        st.session_state.page = "スタートページ"
    # 初期ページを『貸出状況』（= Dashboardページ）に
    if 'initialized_default_page' not in st.session_state:
        st.session_state.page = "スタートページ"
        st.session_state.initialized_default_page = True
    page = st.radio("メニュー", ["スタートページ", "計測器一覧", "熱電対 在庫", "熱電対 使用履歴", "管理者"], key="page")

# Load data
df_instr, df_tc_usage, df_tc_inv = load_data()

# Dashboard（表題のみ『貸出状況』に変更）
if page == "スタートページ":
    st.header("貸出状況")
...
# 管理者
    ...
    # 校正係数列の追加（なければ）＆右端へ配置
    if '校正係数' not in df_master_edit.columns:
        df_master_edit['校正係数'] = pd.Series(dtype='float')
    cols_no_k = [c for c in df_master_edit.columns if c != '校正係数']
    df_master_edit = df_master_edit[cols_no_k + ['校正係数']]

    # Excel由来の不要列（Unnamed:* / 空ヘッダ）を念のため除去
    drop_cols2 = [c for c in df_master_edit.columns if str(c).strip()=='' or str(c).lower().startswith('unnamed:')]
    if drop_cols2:
        df_master_edit = df_master_edit.drop(columns=drop_cols2, errors='ignore')

    # ▼ 表示列はホワイトリストで固定（貸出状況は表示に含める）
    whitelist = [
        '名称','型式','識別番号','貸出状況',
        '貸出日','返却予定日',
        '所属','氏名','使用場所','使用用途',
        '保管場所','備考',
        'メーカー','購入日','校正期限','校正係数1','校正係数1_単位',
    '校正係数2','校正係数2_単位'
    ]
    visible_cols = [c for c in whitelist if c in df_master_edit.columns]
    df_master_for_view = df_master_edit[visible_cols]
...
# ▼▼▼ ここから追記：有効数字を6桁に丸めてからバッファへ反映・保存 ▼▼▼
            for coef_col in ['校正係数1','校正係数2']:
                if coef_col in tmp_df.columns:
...
# ▲▲▲ 追記おわり ▲▲▲
            ...
            # 非表示列を壊さない：元バッファ（base）に列単位で上書きマージ
           
# ▼ 非表示列を壊さない：元バッファに upsert（更新＋追加）
            with st.session_state.autosave_lock:
                base = st.session_state.master_edit_buf.copy()
                key = '識別番号'

    # 1) ID 正規化（両方とも str/strip）
    ...
    # 空やNaNの識別番号は弾く（行を落とす）
                if key in tmp_df.columns:
                    tmp_df = tmp_df[tmp_df[key].notna() & (tmp_df[key].astype(str).str.strip() != '')]

    # 識別番号の重複チェック（編集テーブル側）
                if key in tmp_df.columns and tmp_df[key].astype(str).duplicated().any():
                    st.error('識別番号が重複しています。重複を解消してください。')
                    st.stop()

    # 2) upsert 用に index=識別番号 へ
                if key in base.columns and key in tmp_df.columns:
                    base = base.set_index(key, drop=False)
                    tmpi = tmp_df.set_index(key, drop=False)

        # 列合わせ（列集合の和でそろえる）
                    all_cols = sorted(set(base.columns).union(set(tmpi.columns)))
                    base = base.reindex(columns=all_cols)
                    tmpi = tmpi.reindex(columns=all_cols)

        # 3) 既存ID（共通）を UPDATE
                common_idx = tmpi.index.intersection(base.index)
                if len(common_idx):
                    base.loc[common_idx, :] = tmpi.loc[common_idx, :]

        # 4) 新規ID（baseに無い）を INSERT
                new_idx = tmpi.index.difference(base.index)
                if len(new_idx):
                    base = pd.concat([base, tmpi.loc[new_idx, :]], axis=0)
    ...
    essential_cols = set([
        '名称','型式','識別番号','貸出状況','貸出日','返却予定日',
        '所属','氏名','使用場所','使用用途','保管場所','備考',
        'メーカー','購入日','校正係数1','校正係数1_単位','校正係数2','校正係数2_単位'
    ])
